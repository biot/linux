...
static void rtl83xx_print_matrix(void)
{
	...
	if (soc_info.family == RTL8380_FAMILY_ID) {
		ptr8 = RTL838X_SW_BASE_DSA + RTL838X_PORT_ISO_CTRL(0);
		for (i = 0; i < 28; i += 8)
		...
		ptr9 = RTL838X_SW_BASE_DSA + RTL839X_PORT_ISO_CTRL(0);
		for (i = 0; i < 52; i += 4)
...
static int rtl83xx_setup(struct dsa_switch *ds)
{
	...
	ctrl = sw_r32(priv->r->l2_ctrl_0);
	pr_info("DSA: %s: RTL838X_L2_CTRL_0: %u\n", __func__, ctrl);
	ctrl |= RTL838X_L2_CTRL_0_MC_BC_SA_DROP;
	sw_w32(ctrl, priv->r->l2_ctrl_0);
	ctrl = sw_r32(priv->r->l2_ctrl_0);
	pr_info("DSA: %s: RTL838X_L2_CTRL_0: %u\n", __func__, ctrl);
...
{
	pr_info("%s: dev %s port %d vid %d addr %pM\n", __func__,
		mdb->obj.orig_dev->name, port, mdb->vid, mdb->addr);
	return 0;
}
				   ...
				   bool ingress)
{
	...
	pr_info("In %s: port %d mirror port %d ingress %d\n", __func__, port,
		mirror->to_local_port, ingress);
