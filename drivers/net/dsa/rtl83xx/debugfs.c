				   ...
				   int port)
{
	...
	debugfs_create_x32("rate_uc", 0644, port_dir,
			    (u32 *)(RTL838X_SW_BASE_DSA + RTL838X_STORM_CTRL_PORT_UC(port)));

	debugfs_create_x32("rate_mc", 0644, port_dir,
			    (u32 *)(RTL838X_SW_BASE_DSA + RTL838X_STORM_CTRL_PORT_BC(port)));
	...
	port_ctrl_regset->base = RTL838X_SW_BASE_DSA + (port << 2);
...
void rtl838x_dbgfs_init(struct rtl838x_switch_priv *priv)
{
	...
	debugfs_create_u32("soc", 0444, rtl838x_dir,
			   (u32 *)(RTL838X_SW_BASE_DSA + RTL838X_MODEL_NAME_INFO));
	...
	port_ctrl_regset->regs = port_ctrl_regs;
	port_ctrl_regset->nregs = ARRAY_SIZE(port_ctrl_regs);
	port_ctrl_regset->base = RTL838X_SW_BASE_DSA + (priv->cpu_port << 2);
	debugfs_create_regset32("port_ctrl", 0400, port_dir, port_ctrl_regset);
